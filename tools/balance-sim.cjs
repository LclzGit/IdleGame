...
const HOURS = +process.argv[2] || 12, RAIDS = process.argv[3] === 'raids';   // 'raids': o jogador também enfrenta os Chefões
(async()=>{
const b = await chromium.launch(); const p = await b.newPage();
const errs=[]; p.on('pageerror',e=>errs.push(e.message));
await p.goto('file://'+require('path').join(__dirname, 'sim.html')); await p.waitForTimeout(300);
await p.evaluate(on=>{ window.__raidsOn = on; }, RAIDS);
const rows=[]; const t0=Date.now();
for (let m=1; m<=HOURS*60; m++) {
  await p.evaluate(m=>{ window.__g.step(1200, .05); if (window.__raidsOn) window.__g.raidTick(m); window.__g.player(); }, m);   // 1 minuto de jogo
  if (m % 60 === 0 || m === 10 || m === 30) {
